...
AWS removal done 2026-09-24: `modules/metrics`, `apps/scheduler/create-alerts.ts`, SQS transport, `monitoring` config, `AWS_REGION`, `.platform`, `.ebextensions`, `buildspec.yml` removed; `@aws-sdk/*` deps dropped. Scheduler posts over HTTP only. Subgraph lag job now logs + Sentry warning above `acceptableSGLag`. `stellate/` kept only as reference for the Apollo response-cache rules (still open).
